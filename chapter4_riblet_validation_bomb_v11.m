%% chapter4_riblet_validation_bomb_8.m
...
function run_validation(cfg)
    ...
    xlabelWithCaption(gca, getXLabelText(gca), cfg.objectName + ": STL-derived robust radius profile");
if cfg.saveFigs
    ...
    nCases = numel(cfg.machCases);
    results = repmat(struct('Mach',NaN,'x',[],'tau_w',[],'p_w',[],'q_inf',[],'sPlus',[],'hPlus',[],'p_avg',NaN,'q_avg',NaN,'tau_avg',NaN,'sPlus_avg',NaN,'hPlus_avg',NaN), 1, nCases);
    ...
    for i = 1:nCases
        ...
        % Print ONLY requested values (axial average over mid-body bins to give one value per Mach)
        fprintf('Mach %.2f | p=%.4g Pa | q=%.4g Pa | tau_w=%.4g Pa | s_plus=%.4g | h_plus=%.4g\n', ...
            M, axialAvg(agg.x, p_use), axialAvg(agg.x, q_use), axialAvg(agg.x, tau_use), axialAvg(agg.x, sPlus), axialAvg(agg.x, hPlus));

        % Plot (one figure per Mach increment)
        plotMachFigures(cfg, M, agg.x, tau_use, sPlus, hPlus);

        % Store for optional overlays
        results(i).Mach = M;
        results(i).x      = agg.x;
        results(i).tau_w  = tau_use;
        results(i).p_w    = p_use;
        results(i).q_inf  = q_use;
        results(i).sPlus  = sPlus;
        results(i).hPlus  = hPlus;
        % Axial averages for end-of-run vs-Mach comparison plots
        results(i).p_avg      = axialAvg(agg.x, p_use);
        results(i).q_avg      = axialAvg(agg.x, q_use);
        results(i).tau_avg    = axialAvg(agg.x, tau_use);
        results(i).sPlus_avg  = axialAvg(agg.x, sPlus);
        results(i).hPlus_avg  = axialAvg(agg.x, hPlus);
    end
...
%% ===================== PLOTTING =====================
function plotMachFigures(cfg, M, x_mm, tau_use, sPlus, hPlus)
% One figure per Mach increment (x in mm).
    ...
    xlabelWithCaption(gca, 'x (mm)', cfg.objectName + sprintf(': Riblet wall units (Mach %.2f)', M));
...
legend({'s^+ (unitless)','h^+ (unitless)'}, 'Location','best');
    ...
    if any(~isnan(tau_use))
        ...
        xlabelWithCaption(gca, 'x (mm)', cfg.objectName + sprintf(': \tau_w vs x (Mach %.2f)', M));
    ylabel('\tau_w (Pa)');
if cfg.saveFigs
...
function plotOverlays(cfg, results)
    ...
    if any(arrayfun(@(r) any(~isnan(r.tau_w)), results))
        ...
        xlabelWithCaption(gca, 'x (mm)', cfg.objectName + ": \tau_w overlays");
        ylabel('\tau_w (Pa)');
...
function plotVsMach(cfg, results)
    ...
    for i = 1:n
        x = results(i).x;
        if isfield(results(i),'p_w'),   pA(i)   = axialAvg(x, results(i).p_w);   end
        if isfield(results(i),'q_inf'), qA(i)   = axialAvg(x, results(i).q_inf); end
        if isfield(results(i),'tau_w'), tauA(i) = axialAvg(x, results(i).tau_w); end
        if isfield(results(i),'sPlus'), sA(i)   = axialAvg(x, results(i).sPlus); end
        if isfield(results(i),'hPlus'), hA(i)   = axialAvg(x, results(i).hPlus); end
    end
    ...
    haveP = any(~isnan(pA));
    haveQ = any(~isnan(qA));
    if haveP, plotSmoothVsMach(Ms, pA); end
    if haveQ, plotSmoothVsMach(Ms, qA); end
    xlabelWithCaption(gca, 'Mach', cfg.objectName + " : p and q vs Mach (mid-body avg)");
    ylabel('Pressure (Pa)');
    if haveP && haveQ
    ...
    plotSmoothVsMach(Ms, tauA);
    xlabelWithCaption(gca, 'Mach', cfg.objectName + " : \tau_w vs Mach (mid-body avg)");
    ylabel('\tau_w (Pa)');
    legend({'\tau_w (Pa)'}, 'Location','best');

    % --- Riblet wall units ---
    subplot(3,1,3); grid on; hold on;
    plotSmoothVsMach(Ms, sA);
    plotSmoothVsMach(Ms, hA);
    xlabelWithCaption(gca, 'Mach', cfg.objectName + " : s^+ and h^+ vs Mach (mid-body avg)");
    ylabel('Wall units (unitless)');
...
function ybar = axialAvg(x, y)
% Axial (x-weighted) average across the mid-body bins.
...
function plotSmoothVsMach(Ms, y)
% Markers + shape-preserving curve (pchip). Split at Mach 1 if needed.
    Ms = double(Ms(:));
    y  = double(y(:));
    good = ~isnan(Ms) & ~isnan(y);
    Ms = Ms(good);
    y  = y(good);
    if isempty(Ms)
        return;
    ...
    % Markers only (no straight-line segments)
    plot(Ms, y, 'o', 'LineWidth', 1.2, 'HandleVisibility','off');

    if numel(Ms) < 3
        h = plot(Ms, y, 'o', 'LineWidth', 1.2);
        set(h, 'LineStyle','none');
        return;
    end

    if min(Ms) < 1 && max(Ms) > 1
        Ms1 = Ms(Ms <= 1); y1 = y(Ms <= 1);
        Ms2 = Ms(Ms >= 1); y2 = y(Ms >= 1);
        plotMachSegment(Ms1, y1, true);
        plotMachSegment(Ms2, y2, false);
    else
        plotMachSegment(Ms, y, true);
    end
end

function plotMachSegment(Ms, y, showHandle)
    if numel(Ms) < 2
        h = plot(Ms, y, 'o', 'LineWidth', 1.2);
        set(h, 'LineStyle','none');
        return;
    end
    Mf = linspace(min(Ms), max(Ms), 200);
    yf = interp1(Ms, y, Mf, 'pchip');
    h = plot(Mf, yf, '-', 'LineWidth', 1.4);
    if ~showHandle
...
function base = getXLabelText(ax)
% getXLabelText Return the current x-label text as a char/string.
    if nargin < 1 || isempty(ax); ax = gca; end
    base = ax.XLabel.String;
    if iscell(base)
        base = base{1};
    end
...
function xlabelWithCaption(ax, baseLabel, captionStr)
% xlabelWithCaption Put a "title" BELOW the x-label by using a 2-line xlabel.


    if nargin < 1 || isempty(ax); ax = gca; end
    if nargin < 2; baseLabel = ""; end
    if nargin < 3; captionStr = ""; end

    % Clear any normal title at the top
    title(ax, "");

    baseLabel  = char(string(baseLabel));
    captionStr = char(string(captionStr));

    if strlength(string(captionStr)) == 0
        xlabel(ax, baseLabel, 'Interpreter','tex');
        return;
    end

    % Two-line xlabel: caption appears below the x-label
    xlabel(ax, {baseLabel, captionStr}, 'Interpreter','tex');

    % Give a bit more breathing room at the bottom (helps avoid clipping
    try
        if isempty(getappdata(ax,'OrigPosition'))
            setappdata(ax,'OrigPosition', ax.Position);
        end
        pos0 = getappdata(ax,'OrigPosition');
        dy   = 0.06;
        ax.Position = [pos0(1) pos0(2)+dy pos0(3) pos0(4)-dy];
    catch
    end
end
